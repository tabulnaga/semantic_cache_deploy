...
def _cosine_distance(a: list, b: list) -> float:
    """Compute cosine distance (replaces scipy.spatial.distance.cosine)."""
...
class SemanticCacheWrapper:
    def __init__(
        ...
        # OpenAI API vectorizer (no local model download, Lambda-compatible)
        api_key = os.getenv("OPENAI_API_KEY", "")
        embedding_model = os.getenv("OPENAI_EMBEDDING_MODEL", "text-embedding-3-small")
        self.langcache_embed = OpenAITextVectorizer(
    ...
    def pair_distance(self, question: str, answer: str) -> float:
        ...
        distance = _cosine_distance(q_emb, a_emb)
        return distance
